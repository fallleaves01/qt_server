...
## [消息型](#消息型说明)
+ [私聊消息](#私聊消息-dfriendmessage--传入发送者id接收者id时间和信息内容或直接传入chatmessage类)
+ 私聊表情
+ [群聊消息](#群聊消息-dgroupmessage-传入发送者id群聊id时间和信息内容或直接传入chatmessage类)
+ 群聊表情
...
### 返回 (DUserInfo): 传入请求者uid，用户UserInfo，在线状态
+ type          ：Data::USER_INFO
+ senderUid     ：请求者的Uid
...
### 返回 (DFriendList): 传入请求者uid，用户uid，好友列表
+ type          ：Data::FRIEND_LIST
+ senderUid     ：请求者的Uid
+ userId        ：目标用户的Uid
+ friendList    ：目标用户的好友的UserInfo数组
...
+ groupId       ：目标群聊的Uid
+ time          ：时间
### 返回 (DGroupInfo): 传入请求者uid，群聊GroupInfo，人数
...
### 返回 (DGroupList): 传入请求者uid，用户uid，群聊列表
...
### 返回 (DGroupUser): 传入请求者uid，群聊id，成员列表
